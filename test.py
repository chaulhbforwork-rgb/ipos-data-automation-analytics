...
import ast
import numpy as np
# 1. Khai báo đường dẫn file (sử dụng r'' để tránh lỗi đường dẫn)
path_sale_change_log = r"D:\Làm việc - Bảo Châu\Project\Scraping_data_ban_Pos\Data_sale_change_log\1_Log_Tho_08-04-2026.xlsx"
path_sale_by_date = r"D:\Làm việc - Bảo Châu\Project\Scraping_data_ban_Pos\Data_sale_by_date\2_Hoa_Don_Tho_08-04-2026.xlsx"

# 2. Đọc file vào DataFrame (df)
sale_change_log = pd.read_excel(path_sale_change_log)
sale_by_date = pd.read_excel(path_sale_by_date)

# 3. Kiểm tra thử dữ liệu (Xem 5 dòng đầu)
print("--- Dữ liệu Sale Change Log ---")
print(sale_change_log.head())

print("\n--- Dữ liệu Sale By Date ---")
print(sale_by_date.head())

# 1. Hàm parse an toàn
def parse_toppings(x):
    # Ép về string ngay từ đầu để tránh lỗi float/None không có thuộc tính strip
    ...
    try:
        ...
        import json
        # iPOS thường dùng nháy đơn cho dict, chuẩn JSON cần nháy kép
...
# Bước 4: Normalize và ĐỔI TÊN CỘT TOPPING để tránh ghi đè nhầm ID gốc
toppings_flat = pd.json_normalize(sale_by_date_expanded['toppings_to_explode'])

# QUAN TRỌNG: Chỉ ghi đè những cột hiển thị, KHÔNG ghi đè ID hệ thống
# Danh sách các cột muốn lấy từ topping để hiện ra thay cho món chính
cols_to_update = ['item_name', 'price', 'quantity', 'amount', 'item_id', 'sku']

toppings_flat.index = sale_by_date_expanded.index

# Bước 5: Ghi đè thông minh (Chỉ ghi đè các cột thông tin hiển thị)
for col in toppings_flat.columns:
    if col in cols_to_update and col in sale_by_date_expanded.columns:
        # Nếu là dòng topping (not NaN) thì lấy giá trị topping, ngược lại giữ nguyên món chính
        ...
        # Nếu topping có cột mới hoàn toàn thì thêm vào
        sale_by_date_expanded[col] = toppings_flat[col]

# Bước 6: Đánh dấu Topping cho đẹp
...
# Bước 8: ĐẢM BẢO tran_id LUÔN CÓ (Lấy từ cột id gốc nếu bị thiếu)
# Trong hình của bạn, tran_id dường như là cột id
...
# 1. Hàm trích xuất an toàn từ extra_data
def extract_extra_info(extra_str):
    # Khởi tạo giá trị mặc định nếu lỗi hoặc rỗng
    results = {
        'peo_count': 0,
    ...
    try:
        # Chuyển chuỗi trong extra_data thành dict
...
def get_store_name(x):
    try:
        # Chuyển chuỗi JSON/Dict thành dictionary
        data = ast.literal_eval(str(x))
        # Tìm các trường có thể chứa tên (tùy phiên bản iPOS)
        return data.get('store_name') or data.get('branch_name') or "Không xác định"
    except:
        return "Không xác định"

# Trích xuất ra cột mới
log_final['store_name'] = log_final['extra_data'].apply(get_store_name)

#

# # 1. Định nghĩa bảng ánh xạ (Mapping) từ dữ liệu bạn cung cấp
# STORE_MAP = {
...
# # 2. Tạo cột store_name dựa trên cột store_uid
# # Giả sử DataFrame của bạn tên là df
# log_final['store_name'] = log_final['store_uid'].map(STORE_MAP).fillna("Chi nhánh lạ")

# # 3. Kiểm tra thử kết quả
# print("Thống kê số đơn theo từng quán:")
# print(log_final['store_name'].value_counts())
